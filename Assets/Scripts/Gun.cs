...
public class Gun : MonoBehaviour
{
    ...
    private void Shot(Vector3 startPoint, Vector3 direction)
    {
        ...
        if (Physics.Raycast(startPoint, direction, out hit, fireDistance, ~excludeTarget))
        {
            ...
            {
                EffectManager.Instance.PlayHitEffect(hit.point, hit.normal, hit.transform, EffectManager.EffectType.Common);
            }
    ...
    private IEnumerator ReloadRoutine()
    {
        ...
        ammoRemain -= Mathf.Clamp(magCapacity - magAmmo, 0, ammoRemain);
        magAmmo = magCapacity;
        state = State.Ready;
